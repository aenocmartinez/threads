...
func (c *ComentarioDAO) CrearComentario(comentario *domain.Comentario) bool {
	...
	var padreID interface{}
	if comentario.GetComentarioPadre() != nil {
		padreID = comentario.GetComentarioPadre().GetID()
	} else {
		padreID = nil
	}
	...
	if err != nil {
		fmt.Println("error obteniendo ID del comentario insertado:", err)
		return false
	}

	comentario.SetID(insertedID)
	return true
}
...
func (c *ComentarioDAO) ObtenerComentario(id int64) *domain.Comentario {
	...
	var totalLikes int
	err = c.db.QueryRow(`SELECT COUNT(*) FROM me_gusta_comentario WHERE comentario_id = ?`, id).Scan(&totalLikes)
	if err != nil {
...
func (c *ComentarioDAO) ObtenerComentariosRecientesDesde(fechaUltimo time.Time) []dto.ComentarioConRespuestasDTO {
	...
	if fechaUltimo.IsZero() {
		...
		query = `
			SELECT id FROM comentarios
			WHERE comentario_padre_id IS NULL AND created_at < ?
			ORDER BY created_at DESC
			LIMIT 50
		`
...
func (c *ComentarioDAO) DarMeGustaAComentario(usuarioID, comentarioID int64) bool {
	query := `
		INSERT IGNORE INTO me_gusta_comentario (usuario_id, comentario_id)
		VALUES (?, ?)
	`
